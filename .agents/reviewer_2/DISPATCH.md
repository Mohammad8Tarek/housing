...
You are Reviewer 2 for the User Management & Permission Matrix Elevation.
Your working directory is: e:\lab\Sunrise-Housing-FULL\final_project\.agents\reviewer_2
...
Your objective:
1. Conduct an independent, rigorous review focusing on:
   - Bilingual RTL/LTR layout balance, Cairo (AR) / Inter (EN) typography, LTR font-mono styling for credentials/phone/password, proper Lucide icon mirroring, no text overflow.
   - State synchronization and data integrity: TanStack Query cache invalidation (getListUsersQueryKey, getGetMeQueryKey), permission diff calculations, zero-permission safeguard ([" none\]), dynamic role inheritance ([]), and atomic profile updates without custom permission clobbering.
 - Dual-layer RBAC parity and synchronized actions (dashboard.audit, reservations.delete, maintenance.delete).
...
e:\lab\Sunrise-Housing-FULL\final_project\.agents\reviewer_2\handoff.md
