...
/**
 * Maps CASL Subject names to primary Permission Modules
 */
export function mapSubjectToModule(subject: PolicySubject, resource?: any): PermissionModule {
  switch (subject) {
    case "Ticket":
...
/**
 * Builds a CASL-style ability instance for the given user with fine-grained attribute checks
 */
export function defineAbilityFor(user: any): AppAbility {
  ...
  // Super admin can manage everything unconditionally
  if (isSuperAdmin) {
    rules.push({
      action: "manage",
      subject: "all",
  ...
  const can = (
    ...
    if (subject === "Ticket") {
      ...
      const targetModule: PermissionModule =
        category === "housekeeping" ? "housekeeping" : "maintenance";

      const mappedAction = (action === "manage" ? "edit" : action) as PermissionAction;
