...
async function hasAlreadyActedOnRequest(
  client: any,
  requestId: number,
  userId: number,
): Promise<boolean> {
...
async function getRequestWithSteps(
  requestId: number,
  propertyIds: number[],
  isSystemAdmin: boolean,
) {
  ...
  if (request.assignedRoomId) {
    try {
      ...
      const schemaName =
        propRes.rows[0]?.schema_name || `prop_${row.property_id}`;
      const roomRes = await pool.query(
        `SELECT room_number FROM "${schemaName}".rooms WHERE id = $1`,
...
router.post(
  "/hosting-requests",
  requirePermission("hosting_requests", "create"),
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      try {
        ...
        if (
          err.code === "23505" &&
          err.constraint === "hosting_requests_request_number_key"
        ) {
          res.status(409).json({
            success: false,
            message:
              "رقم الطلب مكرر، يرجى المحاولة مرة أخرى / Duplicate request number, please try again",
          });
          return;
        }
        throw err;
      ...
      const created = await getRequestWithSteps(
        requestId,
        user.propertyIds,
        user.isSystemAdmin,
      );
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      if (q.hotelId) {
        conditions.push(
          `(fvr.hotel_id = $${paramIdx} OR fvr.visit_hotel_id = $${paramIdx})`,
        );
        paramIdx++;
      ...
      const mappedRows = rowsRes.rows.map((row) => {
        ...
        if (row.status === "in_signing" && Array.isArray(row.approval_steps)) {
          const pendingStep = row.approval_steps.find(
...
router.get(
  "/hosting-requests/pending-my-signature",
  requirePermission("hosting_requests", "view"),
  async (req, res): Promise<void> => {
    const user = su(req);
    try {
...
router.post(
  "/hosting-requests/:id/create-guest-hosting",
  requirePermission("hosting_requests", "create"),
  async (req, res): Promise<void> => {
    const user = su(req);
    try {
      ...
      if (!user.propertyId) {
        res
          .status(400)
          .json({ success: false, message: "No property selected" });
        return;
      }
      ...
      try {
        ...
        if (visit.status !== "approved") {
          ...
          res.status(400).json({
            success: false,
            message:
              "يجب الموافقة على طلب الزيارة أولاً / Request must be approved first",
          });
          return;
        }
        ...
        if (visit.guest_hosting_id) {
          ...
          res.status(409).json({
            success: false,
            message:
              "تم إنشاء طلب استضافة مسبقاً / Guest hosting already created",
          });
          return;
        }
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      const request = await getRequestWithSteps(
        requestId,
        user.propertyIds,
        user.isSystemAdmin,
      );
...
router.post(
  "/hosting-requests/:id/sign",
  requirePermission("hosting_requests", "approve"),
  async (req, res): Promise<void> => {
    const user = su(req);
    try {
      const requestId = parseInt(String(req.params.id));
      ...
      try {
        ...
        if (request.status !== "in_signing") {
          ...
          res.status(400).json({
            success: false,
            message: "Request is not in signing status",
          });
          return;
        }
        ...
        if (!hasRole) {
          logger.warn(
            ...
            "Permission denied for signing family visit request",
          );
          await client.query("ROLLBACK");
          res.status(403).json({
            success: false,
            message: `Only ${requiredRole} can sign this step`,
          });
          return;
        }
        ...
        if (sigRes.rows.length === 0 || !sigRes.rows[0]?.signature_image_url) {
          ...
          broadcastToProperty(user.propertyId, {
            ...
            data: {
              ...
              message:
                "You must upload your signature before signing this request.",
              titleAr: "مطلوب توقيع",
          ...
          res.status(400).json({
            success: false,
            message:
              "Upload your signature in Settings before signing requests",
          });
          return;
        }
      ...
      const updated = await getRequestWithSteps(
        requestId,
        user.propertyIds,
        user.isSystemAdmin,
      );
    ...
    try {
      ...
      try {
        ...
        if (!hasRole) {
          ...
          res.status(403).json({
            success: false,
            message: `Only ${requiredRole} can reject this request`,
          });
          return;
        }
...
router.post(
  "/hosting-requests/:id/reback",
  requirePermission("hosting_requests", "edit"),
  async (req, res): Promise<void> => {
    const user = su(req);
    try {
      const requestId = parseInt(String(req.params.id));
      ...
      try {
        ...
        if (!hasRole) {
          ...
          res.status(403).json({
            success: false,
            message: `Only ${requiredRole} can reback this request`,
          });
          return;
        }
      ...
      try {
        ...
        if (
          ...
          res.status(400).json({
            success: false,
            message:
              "Cannot edit this request in its current status unless you are an admin",
          });
          return;
        }
...
router.delete(
  "/hosting-requests/:id",
  requirePermission("hosting_requests", "delete"),
  async (req, res): Promise<void> => {
    const user = su(req);
    try {
      const requestId = parseInt(String(req.params.id));
      ...
      try {
        ...
        await client.query(
          "DELETE FROM public.hosting_request_approval_steps WHERE request_id = $1",
          [requestId],
        );
        await client.query(
          "DELETE FROM public.hosting_requests WHERE id = $1",
          [requestId],
        );
