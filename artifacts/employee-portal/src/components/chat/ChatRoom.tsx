...
import { useConversationMessages, useSendMessage, useMarkAsRead } from "@workspace/api-client-react";
    ...
    <div className="flex flex-col h-full bg-[hsl(var(--background))] absolute inset-0 z-50">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-[hsl(var(--card))] border-b border-[hsl(var(--border2))] shadow-sm sticky top-0 z-10">
        ...
        <div className="relative shrink-0">
          {photoUrl ? (
            <img src={photoUrl} alt={title} className="w-10 h-10 rounded-full object-cover border border-[hsl(var(--border2))]" />
          ) : (
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-[80px]" style={{ backgroundImage: 'var(--chat-bg)', backgroundSize: 'cover' }}>
        {isLoading ? (
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"} mb-4`}>
                ...
                >
                  ...
                  <div className="text-[15px] leading-relaxed break-words" style={{ whiteSpace: "pre-wrap" }}>
                    {msg.content}
                  </div>
                  <div
                    ...
                    {isMe && (
                      <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        {msg.reads && msg.reads.length > 0 ? (
                          // Double tick (read)
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 6L7 17l-5-5m20-2l-7.5 7.5L13 16" className="text-blue-200" />
                        ) : (
                          // Single tick (sent)
