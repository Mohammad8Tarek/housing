  ...
  for (const prop of props.rows) {
    ...
    try {
      const emps = await client.query(
        `SELECT id, employee_id, first_name, last_name, status FROM ${schema}.employees WHERE employee_id = $1`,
        ["10575"],
      );
      if (emps.rows.length > 0) {
        ...
        const accs = await client.query(
          `SELECT id, employee_id, is_active, must_change_password, failed_attempts FROM ${schema}.employee_portal_accounts WHERE employee_id = $1`,
          ["10575"],
        );
