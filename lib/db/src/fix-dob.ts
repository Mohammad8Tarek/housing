...
async function check() {
  ...
  try {
    ...
    for (const schema of schemas) {
      try {
        const cols = await client.query(`SELECT column_name FROM information_schema.columns WHERE table_schema = $1 AND table_name = 'profiles'`, [schema]);
        if (cols.rows.length > 0) {
          const hasDob = cols.rows.some(r => r.column_name === 'date_of_birth');
          console.log(`${schema}.profiles: ${hasDob ? 'HAS date_of_birth' : 'MISSING date_of_birth'}`);
          
          if (!hasDob) {
            console.log(`Fixing ${schema}...`);
            await client.query(`ALTER TABLE "${schema}"."profiles" ADD COLUMN IF NOT EXISTS "date_of_birth" text DEFAULT '' NOT NULL;`);
