...
export const hostingCompanionsTable = pgTable("hosting_companions", {
  ...
  hostingId: integer("hosting_id").notNull().references(() => hostingsTable.id, { onDelete: "cascade" }),
...
export const insertHostingCompanionSchema = createInsertSchema(hostingCompanionsTable).omit({ id: true, createdAt: true });
export type InsertHostingCompanion = z.infer<typeof insertHostingCompanionSchema>;
