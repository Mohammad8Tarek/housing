...
export const portalTransportSchedulesTable = pgTable("portal_transport_schedules", {
  id:           serial("id").primaryKey(),
  propertyId:   integer("property_id").notNull(),
...
export const portalTransportBookingsTable = pgTable("portal_transport_bookings", {
  id:           serial("id").primaryKey(),
  propertyId:   integer("property_id").notNull(),
  employeeId:   integer("employee_id").notNull(),
...
export const insertFoodMenuSchema = createInsertSchema(portalFoodMenuTable).omit({ id: true, createdAt: true });
export const insertMealOrderSchema = createInsertSchema(portalMealOrdersTable).omit({ id: true, createdAt: true }).extend({
  menuItemId: z.number(),
  quantity: z.number().min(1).default(1),
  orderDate: z.string(),
});
export const insertTransportScheduleSchema = createInsertSchema(portalTransportSchedulesTable).omit({ id: true, createdAt: true });
export const insertTransportBookingSchema = createInsertSchema(portalTransportBookingsTable).omit({ id: true, createdAt: true });
...
export type TransportSchedule = typeof portalTransportSchedulesTable.$inferSelect;
