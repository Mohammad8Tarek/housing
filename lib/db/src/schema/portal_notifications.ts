...
export const portalNotificationReadsTable = pgTable("portal_notification_reads", {
  id:             serial("id").primaryKey(),
  notificationId: integer("notification_id").notNull(),
  employeeId:     integer("employee_id").notNull(),
  readAt:         timestamp("read_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => [
  uniqueIndex("uq_portal_notification_reads").on(t.notificationId, t.employeeId),
]);
...
export const insertPortalNotificationSchema = createInsertSchema(portalNotificationsTable).omit({ id: true, createdAt: true });
