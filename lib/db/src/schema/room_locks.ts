...
export const keyAuditLogTable = pgTable("key_audit_log", {
  id: serial("id").primaryKey(),
  propertyId: integer("property_id").notNull(),
  keyId: integer("key_id").references(() => roomKeysTable.id, {
    onDelete: "set null",
  }),
...
export const insertRoomKeySchema = createInsertSchema(roomKeysTable).omit({
  id: true,
  issuedAt: true,
  revokedAt: true,
});
export const insertKeyAuditLogSchema = createInsertSchema(
  keyAuditLogTable,
